...
def visualise(iterations, filenameBase="gastrulation_animation", save=True, show=False):
    ...
    def update(frame_idx):
        ...
        scatter = ax.scatter(x, y, z, c=values, cmap="viridis", alpha=0.8, s=2)
    ...
    if save:
        ani = animation.FuncAnimation(
            fig, update, frames=len(tensors_array), interval=200, blit=False
        )
...
def preprocessing():
    ...
    for axis in ["x", "y", "z"]:
        print(axis)
        data[axis] = (
            GRID_SIZE_0_INDEXING * (data[axis] - global_min) / (global_max - global_min)
        )
    ...
    for index, dataframe in enumerate(iterations):
        ...
        for i, row in dataframe.iterrows():
            ...
            if (
                x < 0
                or x >= GRID_SIZE_0_INDEXING
                or y < 0
                or y >= GRID_SIZE_0_INDEXING
                or z < 0
                or z >= GRID_SIZE_0_INDEXING
            ):
                print(
                    f"Out of bounds: x={x}, y={y}, z={z} for index {index} in iteration {index}"
                )
                continue
    ...
    for tensor in voxels:
        ...
        input_image[:, 0, :, :, :] = (
            tensor  # Assign the 3D tensor to the first channel (alpha mask)
        )
