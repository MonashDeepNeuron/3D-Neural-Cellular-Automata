...
ITERATION = 18934
df = pd.read_csv(csv_path)[:ITERATION]

# Inspect the data
# I want to get the length of the data
#print(df.shape) # between 18 and 19k rows
#print(df.head(18800))
...
arr_swapped = arr[:, [2, 1, 0]]
print("swapped shape", arr_swapped.shape)
#print(arr, "\n", arr_swapped)

# Get the maximum values for each axis
max_z, max_y, max_x = np.max(arr_swapped, axis=0).astype(int)

scale_factor = 10  # Reduce resolution
...
for z, y, x in arr_swapped.astype(int):
    voxel_grid[z, y, x] = 1
...
# Now pass voxel_grid to plot_single_instance

#grid = plot_single_instance(voxel_grid, th=0.1, opacity=True)
#print(len(grid))
# plot_voxel_grid(grid, opacity=True)
#plot_3d(voxel_grid)
