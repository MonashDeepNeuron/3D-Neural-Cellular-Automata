...
"""
    ...
    Hence, the expected shape of the input is [x, y, z, 4]. We can add additional channels if we want to encode additional hidden 
    information, thus making the expected shape of the input to be [x, y, z, C]
    
    If we perform batching (or in our case, sampling), the expected shape of the input becomes [b, x, y, z, C]. This is what we
    assume we are given.
"""
...
class NCA3DModel(torch.nn.Module):
    ...
    def alive(self, x):
        ...
        """
        alpha = x[..., 3:4].permute(0, 4, 1, 2, 3).contiguous()
        return F.max_pool3d(
            alpha,  # [b,4,x,y,z] -> Alpha channel is the 4th channel
            kernel_size=3,
            stride=1,
            padding=1,
    ...
    def generate_gif(self, x, shape, channels):
        ...
        frames_array = torch.zeros((random_number_steps,
            shape[0],
            shape[1],
            shape[2],
            channels
        ...
        frames_array[0] = x.permute(0,2,3,4,1)
        for step in range(1, random_number_steps):
            frames_array[step] = self.update(frames_array[step-1].unsqueeze(0))
        return frames_array


# For Indexing a tensor that is contiguous [x,y,z,c], and only works if its [x,y,z,c] not [c,x,y,z]:
    # index = z * y_size * x_size * channels + y * x_size * channels + x * channels + c
    # value = x.view(-1)[index]
